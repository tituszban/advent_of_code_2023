...
def solve(input_lines: list[str]):
    ...
    for brick in bricks:
        ...
        brick_z[brick] = max_height + 1

    supporting = set()

    for brick in bricks:
        if not all(len(b.supported_by) > 1 for b in brick.supporting):
            supporting.add(brick)

    total = 0
    for brick in sorted(supporting, key=lambda b: brick_z[b]):
        falling = set([brick])

        def _check_brick(brick_to_check: "Brick"):
            new_falling = set()
            for child in brick_to_check.supporting:
                if all(s in falling for s in child.supported_by):
                    new_falling.add(child)
